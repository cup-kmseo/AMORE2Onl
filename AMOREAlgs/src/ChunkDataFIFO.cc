...
int ChunkDataFIFO::PushChunk(unsigned short ** adc, unsigned long * time, int ndp)
{
  if (ndp <= 0) return -1;
  auto chunk = std::make_unique<Chunk>(fNChannel, ndp);
...
int ChunkDataFIFO::PopCurrent(unsigned short * adc, unsigned long & time)
{
...
int ChunkDataFIFO::DumpCurrent(unsigned short ** outADC, unsigned long * outTime)
{
  ...
  if (toCopyFromCurrent > 0) {
    ...
    for (int ch = 0; ch < fNChannel; ++ch)
      std::memcpy(&outADC[ch][copied], &fCurrentChunk->fADC[ch][startIdx],
                  toCopyFromCurrent * sizeof(unsigned short));
  ...
  if (copied < totalNDP) {
    ...
    if (fNextChunk) {
      ...
      for (int ch = 0; ch < fNChannel; ++ch)
        std::memcpy(&outADC[ch][copied], &fNextChunk->fADC[ch][0],
                    toCopyFromNext * sizeof(unsigned short));
    ...
    if (tailRemaining > 0) {
      ...
      for (int ch = 0; ch < fNChannel; ++ch)
        std::memset(&outADC[ch][copied], 0, tailRemaining * sizeof(unsigned short));
